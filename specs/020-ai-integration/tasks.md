...
- [X] T032 [P] Run `pnpm --filter @fieldframe/web typecheck` and `pnpm --filter @fieldframe/worker typecheck` (and `pnpm --filter @fieldframe/queue typecheck` for T004) across every file touched above. **Done** — clean on every AI file; remaining failures are a pre-existing, unrelated `TextDocument`→`TextAsset` refactor in progress elsewhere.
