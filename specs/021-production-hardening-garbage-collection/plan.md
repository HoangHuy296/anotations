...
**Primary Dependencies**: Next.js 16 (App Router, `apps/web`) · Prisma 6 / `@prisma/client` (PostgreSQL) · BullMQ 5 + ioredis 5 (`apps/worker`, queue transport) · `minio` SDK 8 (object storage, used by both apps) · Zod 4 (request/config validation) · React 19 + react-konva (canvas, unaffected) · `@fieldframe/domain` (shared provider config/readiness) and `@fieldframe/queue` (shared queue contract types) workspace packages.
