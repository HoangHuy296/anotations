...
- [X] T030 Run `pnpm typecheck`, `pnpm --filter @annotationplatform/web lint`, `pnpm --filter @annotationplatform/web test:direct-upload`, and the Compose runtime validation from `specs/006-direct-upload-multi-modal-metadata-rows/quickstart.md`; record exact pass/fail results in that quickstart.
