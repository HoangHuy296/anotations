...
test("cleanup removes only unreferenced objects inside its import prefix and is idempotent", { skip: !hasImportIntegration }, async () => {
  configureLocalImportHostMinio();
  const fixture = await createPreparedImportFixture();
  const { config, minio } = getDirectUploadProviders();
  const prefix = `prepared-imports/${fixture.preparedImport.id}/`;
  const orphan = `${prefix}orphan`;
  const published = `${prefix}published`;
  const outside = "prepared-imports/another-import/outside";
  try {
    ...
    const first = await cleanupPreparedImportOrphans({ minio, bucket: config.MINIO_BUCKET, preparedImportId: fixture.preparedImport.id });
    const second = await cleanupPreparedImportOrphans({ minio, bucket: config.MINIO_BUCKET, preparedImportId: fixture.preparedImport.id });
