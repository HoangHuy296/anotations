...
async function assertDenied(cookie: string | null, path: "/api/source-import-preflight" | "/api/datasets/from-repository", body: unknown, expected: { status: number; code: string }, sentinels: string[] = []) {
...
test("source-import credential-mode denials are concealed, redacted, and side-effect free", {
  ...
  try {
    ...
    await assertDenied(actor.cookie, "/api/source-import-preflight", { ...base, credentialMode: "EXISTING_SOURCE_CONNECTION", sourceConnectionId: foreignConnectionId, personalAccessToken: "sentinel-pat" }, { status: 400, code: "INVALID_REQUEST" }, ["sentinel-pat"]);
    ...
    await assertDenied(actor.cookie, "/api/datasets/from-repository", {
      provider: "GITEA",
      datasetName: base.datasetName,
      credentialMode: "ONE_TIME_PAT",
      serverUrl: "http://localhost:3100",
      personalAccessToken: "one-time-sentinel",
      ...
      idempotencyKey: "phase015-unsaved-one-time-pat-denial",
    }, { status: 422, code: "ONE_TIME_PAT_REQUIRES_SAVE_FOR_ASYNC_IMPORT" }, ["one-time-sentinel"]);
...
test("pre-policy Gitea source-import denials make zero provider calls by access-log evidence", {
  ...
  try {
    ...
    const denials: Array<{ body: unknown; expected: { status: number; code: string }; sentinels?: string[] }> = [
      ...
      { body: { ...base, credentialMode: "EXISTING_SOURCE_CONNECTION", sourceConnectionId: foreignConnectionId, personalAccessToken: "fixture-sentinel" }, expected: { status: 400, code: "INVALID_REQUEST" }, sentinels: ["fixture-sentinel"] },
