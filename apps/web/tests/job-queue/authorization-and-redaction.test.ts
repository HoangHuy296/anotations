...
test("denied, malformed, cross-dataset, and unsupported submissions write no Job", { skip: queueIntegrationSkipReason }, async () => {
  ...
  try {
    ...
    const unsupported = await createAndEnqueueFoundationJob(fixture.owner, { datasetId: fixture.datasetId, type: JobType.AI_TASK_SYNC, input: {} });
...
test("authorized application cancellation terminally cancels an unclaimed queued Job", { skip: queueIntegrationSkipReason }, async () => {
  const fixture = await createJobQueueFixture();
  try {
    const job = await fixture.createQueuedJob();
    assert.deepEqual(await cancelAuthorizedJob(fixture.owner, job.id), { ok: true, status: 200, cancellationStatus: "CANCELED" });
