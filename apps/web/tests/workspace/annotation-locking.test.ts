...
test("concurrent Annotation edits allow one current revision and preserve the winner", { skip: !enabled }, async () => {
  ...
  try {
    ...
    const [first, stale] = await Promise.all([
      updateBoundingBoxGeometry(owner, { datasetId: dataset.id, assetId: asset.id, annotationId: created.value.id, revision: created.value.revision, geometry: { x: 0.2, y: 0.1, width: 0.2, height: 0.2 } }),
      updateBoundingBoxGeometry(manager, { datasetId: dataset.id, assetId: asset.id, annotationId: created.value.id, revision: created.value.revision, geometry: { x: 0.3, y: 0.1, width: 0.2, height: 0.2 } }),
    ]);
    ...
    assert.ok(winner);
    const labelStale = await updateBoundingBoxLabel(manager, { datasetId: dataset.id, assetId: asset.id, annotationId: created.value.id, revision: created.value.revision, labelId: null });
    assert.deepEqual(labelStale, { ok: false, status: 409 });
    const deleteStale = await deleteBoundingBox(manager, { datasetId: dataset.id, assetId: asset.id, annotationId: created.value.id, revision: created.value.revision });
