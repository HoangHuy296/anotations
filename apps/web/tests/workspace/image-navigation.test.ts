...
test("selected Asset reconciliation and previous/next stay in a filtered order across page boundaries", { skip: !enabled }, async () => {
  ...
  try {
    ...
    const selected = await readImageWorkspacePage(owner, dataset.id, {
      page: 1, search: "FILTERED", statuses: [AssetStatus.COMPLETED], selectedAssetId: assets[100]!.id,
    });
    assert.equal(selected?.page.page, 1);
    // The one NEW item is excluded, so original index 100 is filtered index 99.
    assert.equal(selected?.page.selectedAssetId, assets[100]!.id);
    ...
    const empty = await readImageWorkspacePage(owner, dataset.id, { search: "does-not-exist", selectedAssetId: assets[0]!.id });
    assert.equal(empty?.page.total, 0);
    assert.equal(empty?.page.selectedAssetId, null);
