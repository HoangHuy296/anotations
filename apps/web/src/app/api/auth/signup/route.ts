...
// Deliberately does not create a session: registration only creates the
// account. The caller signs in afterward through the normal /login flow.
export async function POST(request: Request) {
  ...
  try {
    ...
    return apiSuccess({ id: user.id, email: user.email, name: user.name ?? user.email, role: user.role }, { status: 201 });
  } catch (error) {
