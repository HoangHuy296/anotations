...
export default function CanvasStage({ image: asset, annotations: initialAnnotations, unsupportedAnnotations, labels, tool, onToolChange }: CanvasStageProps) {
  ...
  async function persistCreate(type: SafeImageAnnotation["type"], geometry: ImageAnnotationGeometry) {
    setSaveState("saving");
    ...
    if (created) { upsertSafeAnnotation(created); setSelectedId(created.id); onToolChange("select"); setSaveState("saved"); return "saved" as const; }
  ...
  function finishPath() {
    ...
    setPathPoints([]);
    scheduleAutosave(`create-${pendingCreateCounterRef.current++}`, () => persistCreate(type, geometry));
  }
  ...
  function startCanvasInteraction(event: Konva.KonvaEventObject<MouseEvent>) {
    ...
    if (tool === "point") {
      scheduleAutosave(`create-${pendingCreateCounterRef.current++}`, () => persistCreate("POINT", { px: point.x / originalWidth, py: point.y / originalHeight }));
      return;
    }
  ...
  function finishCanvasInteraction() {
    ...
    if (tool === "box") {
      ...
      if (geometry) scheduleAutosave(`create-${pendingCreateCounterRef.current++}`, () => persistCreate("BOUNDING_BOX", geometry));
    } else if (tool === "circle") {
      ...
      if (r > 0 && cx - r >= 0 && cx + r <= 1 && cy - r >= 0 && cy + r <= 1) scheduleAutosave(`create-${pendingCreateCounterRef.current++}`, () => persistCreate("CIRCLE", { cx, cy, r }));
    }
    setDraft(null);
          ...
          {annotations.map((annotation) => {
            ...
            if ((annotation.type === "POLYGON" || annotation.type === "POLYLINE")) {
              ...
              return <Group key={annotation.id}>
                <Line points={points} closed={annotation.type === "POLYGON"} draggable={tool === "select" && selected} {...common} onDragEnd={persistTranslation} />
                {selected && tool === "select" && normalized.map(([x, y], index) => <Circle key={`${annotation.id}-vertex-${index}`} x={x * originalWidth} y={y * originalHeight} radius={5} fill="#ffffff" stroke={colorFor(annotation, labels)} strokeWidth={2} draggable onDragEnd={(event) => {
            ...
            if (annotation.type === "CIRCLE" && typeof geometry.cx === "number" && typeof geometry.cy === "number" && typeof geometry.r === "number") {
              ...
              return <Group key={annotation.id}>
                <Circle x={circle.cx * originalWidth} y={circle.cy * originalHeight} radius={circle.r * originalWidth} draggable={tool === "select" && selected} {...common} onDragEnd={persistTranslation} />
                {selected && tool === "select" && <Circle x={(circle.cx + circle.r) * originalWidth} y={circle.cy * originalHeight} radius={5} fill="#ffffff" stroke={colorFor(annotation, labels)} strokeWidth={2} draggable onDragEnd={(event) => {
            ...
            if (annotation.type === "POINT" && typeof geometry.px === "number" && typeof geometry.py === "number") return <Circle key={annotation.id} x={geometry.px * originalWidth} y={geometry.py * originalHeight} radius={5} fill={colorFor(annotation, labels)} draggable={tool === "select" && selected} {...common} onDragEnd={persistTranslation} />;
            return null;
          })}
