...
import { FileAudioIcon, SpeakerHigh, Cursor, Hand, StampIcon, NotePencilIcon, HighlighterIcon } from "@phosphor-icons/react";
import type { ReactNode } from "react";

import { useAnnotationStore } from "@/stores/image-annotation-store";

/**
 * Audio's `DatasetSidebar` toolbox content. This is the pre-registry
 * `DatasetSidebar` tool-button markup, unchanged in behavior, moved here so
 * `workspace-engine-registry.ts` can reference it as the Audio entry's
...
export function AudioToolbox() {
  ...
  return <>
    <div className="mt-3 grid grid-cols-3 gap-1.5">
      ...
      <ToolButton active={tool === "timesegmentaudio"} label="Time Segment" onClick={() => setTool("timesegmentaudio")}><FileAudioIcon size={17} /></ToolButton>
    </div>
    <div className="mt-2 grid grid-cols-2 gap-1.5">
      <ToolButton active={tool === "timestampaudio"} label="Timestamp Label" onClick={() => setTool("timestampaudio")}><StampIcon size={15} /></ToolButton>
      <ToolButton active={tool === "speakerlabel"} label="Speaker" onClick={() => setTool("speakerlabel")}><SpeakerHigh size={15} /></ToolButton>
