...
export function ImagePropertiesTabs({ datasetId, selection, assets, page, pageSize, totalAssets, completedAssets, search, statuses, selectedAssetId, tab, setTab }: ImagePropertiesTabsProps) {
  ...
  /** Seeds a fresh draft only on the collapsed->expanded transition, mirroring `VideoPropertiesTabs`' track-edit pattern -- so an in-progress edit is never silently clobbered by the label list refreshing elsewhere while its card stays open. */
