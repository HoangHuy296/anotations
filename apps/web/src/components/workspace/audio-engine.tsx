...
type AudioEngineProps = { asset: { id: string; filename: string; description: string | null }; readiness: SafeMediaReadiness };

/** Safe read-only AUDIO workspace; waveform bytes/keys never reach the browser DTO. */
export function AudioEngine({ asset, readiness }: AudioEngineProps) {
  ...
  return <section className="canvas-grid flex min-h-[520px] min-w-0 flex-col items-center justify-center bg-zinc-950 px-6 text-center text-zinc-100 lg:min-h-0">
    ...
    <p className="mt-1 text-xs text-zinc-400">{asset.filename} · {readiness.state.replaceAll("_", " ")}</p>
