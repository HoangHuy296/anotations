...
export function LocalFolderImportForm() {
  ...
  async function scan() {
    ...
    try {
      ...
      setFiles(entries);
      if (!name && entries.length) setName(entries[0]?.logicalPath.split("/")[0] ?? "Local folder");
      setStep("idle");
    } catch {
  ...
  async function submit() {
    ...
    try {
      const started = await fetch("/api/imports/local-folder", {
        method: "POST", headers: { "content-type": "application/json" }, credentials: "same-origin",
        body: JSON.stringify({ name: name.trim(), idempotencyKey: crypto.randomUUID() + crypto.randomUUID(), items: files.map(({ file, logicalPath, fingerprint }) => ({ logicalPath, contentType: file.type || "text/plain", sizeBytes: file.size, fingerprint })) }),
      });
  ...
  return <div className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 lg:py-12">
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_320px]">
      <section className="min-w-0">
        ...
        <h1 className="mt-3 max-w-2xl text-4xl font-bold tracking-[-0.05em] text-zinc-950 sm:text-5xl">Bring a folder into a new Dataset.</h1>
        ...
        <div className="mt-7 flex flex-wrap gap-3"><Button type="button" disabled={busy || files.length === 0 || !name.trim()} onClick={() => void submit()}>{busy ? <><SpinnerGap className="animate-spin" size={17} />{step === "scanning" ? "Scanning folder…" : step === "preparing" ? "Preparing import…" : "Uploading files…"}</> : <><CloudArrowUp size={18} weight="bold" />Upload folder</>}</Button>{jobId && <Button asChild variant="secondary"><Link href={`/jobs/${jobId}`}>Open import job</Link></Button>}</div>
      </section>
